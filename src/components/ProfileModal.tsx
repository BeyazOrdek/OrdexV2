  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="ordex-panel-2 max-h-[90vh] max-w-md overflow-y-auto border-white/10">
        ...
        {/* Radix unmounts this subtree when the dialog closes, so every field
            re-seeds from the latest user row on each open — no setState inside
            an effect, no cascading renders. */}
        <ProfileForm user={user ?? null} onSaved={() => onOpenChange(false)} />
      </DialogContent>
    </Dialog>
  );
}

function ProfileForm({ user, onSaved }: { user: ProfileUser | null; onSaved: () => void }) {
